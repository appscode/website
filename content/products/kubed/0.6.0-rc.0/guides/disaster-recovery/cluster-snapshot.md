...
> New to Kubed? Please start [here](/products/kubed/0.6.0-rc.0/concepts/README).
...
Now, deploy Kubed operator in your cluster following the steps [here](/products/kubed/0.6.0-rc.0/setup/install). Once the operator pod is running, check your bucket from Google Cloud console. You should see the data from initial snapshot operation.

![GCS Snapshot](/products/kubed/0.6.0-rc.0/images/cluster-snapshot/gcs-snapshot.png)
...
Now, deploy Kubed operator in your cluster following the steps [here](/products/kubed/0.6.0-rc.0/setup/install). Once the operator pod is running, check your bucket from S3 console. You should see the data from initial snapshot operation.
...
Now, deploy Kubed operator in your cluster following the steps [here](/products/kubed/0.6.0-rc.0/setup/install). Once the operator pod is running, check your container from Azure portal. You should see the data from initial snapshot operation.
...
Now, deploy Kubed operator in your cluster following the steps [here](/products/kubed/0.6.0-rc.0/setup/install). Once the operator pod is running, check your container. You should see the data from initial snapshot operation.
...
Now, deploy Kubed operator in your cluster. Since, `snapshotter.local.path` is set to `/var/data`, mount your local volume at that path. You can find example installation scripts below:
 - [Without RBAC](/products/kubed/0.6.0-rc.0/examples/cluster-snapshot/local/without-rbac.yaml)
 - [With RBAC](/products/kubed/0.6.0-rc.0/examples/cluster-snapshot/local/with-rbac.yaml)
...
## Cleaning up
To cleanup the Kubernetes resources created by this tutorial, uninstall Kubed operator following the steps [here](/products/kubed/0.6.0-rc.0/setup/uninstall).


## Next Steps
 - To setup a recycle bin for deleted and/or updated Kubernetes objects, please visit [here](/products/kubed/0.6.0-rc.0/guides/disaster-recovery/recycle-bin).
 - Backup your persistent volumes with [Stash](/products/kubed/0.6.0-rc.0/guides/disaster-recovery/stash).
 - Need to keep configmaps/secrets synchronized across namespaces or clusters? Try [Kubed config syncer](/products/kubed/0.6.0-rc.0/guides/config-syncer/).
 - Want to keep an eye on your cluster with automated notifications? Setup Kubed [event forwarder](/products/kubed/0.6.0-rc.0/guides/cluster-events/).
 - Out of disk space because of too much logs in Elasticsearch or metrics in InfluxDB? Configure [janitors](/products/kubed/0.6.0-rc.0/guides/janitors) to delete old data.
 - Wondering what features are coming next? Please visit [here](/products/kubed/0.6.0-rc.0/roadmap).
 - Want to hack on Kubed? Check our [contribution guidelines](/products/kubed/0.6.0-rc.0/CONTRIBUTING).
