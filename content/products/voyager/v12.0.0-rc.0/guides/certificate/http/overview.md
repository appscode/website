...
Install Voyager operator in your cluster following the steps [here](/products/voyager/v12.0.0-rc.0/setup/install).
...
This secret must not have any dashes or other special characters.
    ...
    If you look at the Ingress, you should see that `/.well-known/acme-challenge/` path has been added to rules. It should look like [this](/products/voyager/v12.0.0-rc.0/examples/certificate/http/ing-with-acme-path.yaml).
    
    If you check the configmap `voyager-test-ingress`, you should see a key `haproxy.cfg` with the value similar to [this](/products/voyager/v12.0.0-rc.0/examples/certificate/http/haproxy-with-acme.cfg).
...
9. Now edit the Ingress to add `spec.tls` section.
    ...
    After editing, your Ingress should look similar to [this](/products/voyager/v12.0.0-rc.0/examples/certificate/http/ing-tls-acme.yaml).

10. Now wait several seconds for HAProxy to reconfigure. If you check the configmap `voyager-test-ingress`, you should see a key `haproxy.cfg` with the value similar to [this](/products/voyager/v12.0.0-rc.0/examples/certificate/http/haproxy-ssl.cfg).
