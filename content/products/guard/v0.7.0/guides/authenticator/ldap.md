...
Guard installation guide can be found [here](/products/guard/v0.7.0/setup/install). To use LDAP, create a client cert with `Organization` set to `Ldap`. For LDAP `CommonName` is optional. To ease this process, use the Guard cli to issue a client cert/key pair.
