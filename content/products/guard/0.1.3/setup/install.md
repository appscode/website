...
> New to Guard? Please start [here](/products/guard/0.1.3/concepts).
...
If you want to set up guard via [__Kops__](https://github.com/kubernetes/kops) visit [here](/products/guard/0.1.3/setup/install-kops) to see differences in setup.

If you want to set up guard via [__Kubespray__](https://github.com/kubernetes-incubator/kubespray) visit [here](/products/guard/0.1.3/setup/install-kubespray).
...
As you can see, Guard stores the generated certificates in `.guard` subdirectory of home directory of user executing these commands. You can change the location by passing the path to a different directory using [`--pki-dir`](https://github.com/appscode/guard/pull/25) flag. Guard can use [supported authenticator](/products/guard/0.1.3/guides/) to authenticate users for a Kubernetes cluster. A Kubernetes cluster can use one of these organization to authenticate users. But you can configure a single Guard server to perform authentication for multiple clusters, where each cluster uses a different auth provider.
