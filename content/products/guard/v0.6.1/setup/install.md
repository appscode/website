...
> New to Guard? Please start [here](/products/guard/v0.6.1/concepts).
...
If you want to set up guard via [__Kops__](https://github.com/kubernetes/kops) visit [here](/products/guard/v0.6.1/setup/install-kops) to see differences in setup.

If you want to set up guard via [__Kubespray__](https://github.com/kubernetes-incubator/kubespray) visit [here](/products/guard/v0.6.1/setup/install-kubespray).
...
Guard can use [supported authenticator](/products/guard/v0.6.1/guides/) to authenticate users for a Kubernetes cluster. A Kubernetes cluster can use one of these organization to authenticate users. But you can configure a single Guard server to perform authentication for multiple clusters, where each cluster uses a different auth provider.
