...
If you are upgrading Stash to a patch release, please reapply the [installation instructions](/products/stash/0.6.4/setup/install). That will upgrade the operator pod to the new version and fix any RBAC issues.

## Upgrading from 0.5.x to 0.6.x

The format for `Restic` object has changed in backward incompatiable manner between 0.5.x and 0.6.4 . The steps involved in upgrading Stash operator to 0.6.4 from prior version involves the following steps:
...
Uninstall old `Stash` operator by following the instructions [here](/products/stash/0.6.4/setup/uninstall).
...
Install new `Stash` operator by following the instructions [here](/products/stash/0.6.4/setup/install).
