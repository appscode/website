...
menu:
  product_stash_v0.9.0-rc.1:
    identifier: standalone-elasticsearch-6.2
    name: Backup & Restore Elasticsearch
    parent: stash-elasticsearch-guides-6.2
    weight: 10
product_name: stash
...
sample-elasticsearch   6.2-v1    Running   3m35s
```
...
sample-elasticsearch-backup   elasticsearch-backup-6.2   */5 * * * *   true     3m8s
```
...
NAME                     VERSION   STATUS         AGE
restored-elasticsearch   6.2-v1    Initializing   38s
```
...
spec:
  task:
    name: elasticsearch-restore-6.2
  repository:
    name: gcs-repo
  target:
    ref:
...
$ kubectl get es -n demo restored-elasticsearch
NAME                     VERSION   STATUS    AGE
restored-elasticsearch   6.2-v1    Running   2m16s
```
