...
menu:
  product_stash_v0.9.0-rc.1:
    identifier: standalone-mongodb-4.0.5
    name: Standalone MongoDB
    parent: stash-mongodb-guides-4.0.5
    weight: 10
product_name: stash
...
- Install MongoDB addon for Stash following the steps [here](/products/stash/v0.9.0-rc.1/addons/mongodb/setup/install).
...
- If you are not familiar with how Stash backup and restore MongoDB databases, please check the following guide [here](/products/stash/v0.9.0-rc.1/addons/mongodb/overview).
...
sample-mongodb   4.0.5-v2      Running   2m9s
```
...
Here, we have to use service `sample-mongodb` and secret `sample-mongodb-auth` to connect with the database. KubeDB creates an [AppBinding](/products/stash/v0.9.0-rc.1/concepts/crds/appbinding) crd that holds the necessary information to connect with the database.
...
metadata:
  labels:
    ...
    app.kubernetes.io/version: 4.0.5-v2
...
sample-mongodb-backup  mongodb-backup-4.0.5        */5 * * * *   true     26m
```
...
NAME               VERSION       STATUS         AGE
restored-mongodb   4.0.5-v2      Initializing   17s
```
...
$ kubectl get mg -n demo restored-mongodb
NAME               VERSION       STATUS    AGE
restored-mongodb   4.0.5-v2      Running   105m
```
