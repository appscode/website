...
`spec.type` is an optional field that indicates the type of the app that this `AppBinding` is pointing to. Stash uses this field to resolve the values of `TARGET_APP_TYPE`, `TARGET_APP_GROUP` and `TARGET_APP_RESOURCE` variables of [BackupBlueprint](/products/stash/v0.9.0-rc.2/concepts/crds/backupblueprint) object.
...
- Learn how to backup/restore databases using Stash from [here](/products/stash/v0.9.0-rc.2/guides/latest/databases/overview).
