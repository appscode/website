...
menu:
  product_stash_v0.9.0-rc.2:
    identifier: standalone-elasticsearch-7.2
    name: Backup & Restore Elasticsearch
    parent: stash-elasticsearch-guides-7.2
    weight: 10
product_name: stash
...
sample-elasticsearch   7.2           Running   3m35s
```
...
sample-elasticsearch-backup   elasticsearch-backup-7.2     */5 * * * *   true     3m8s
```
...
NAME                     VERSION     STATUS         AGE
restored-elasticsearch   7.2         Initializing   38s
```
...
spec:
  task:
    name: elasticsearch-restore-7.2
  repository:
    name: gcs-repo
  target:
    ref:
...
$ kubectl get es -n demo restored-elasticsearch
NAME                     VERSION     STATUS    AGE
restored-elasticsearch   7.2         Running   2m16s
```
